...
class LLMDecisionMaker:
    ...
    def _format_history(self, history: list) -> str:
        ...
        for trial in history:
            ...
            if 'vram_red' in metrics:
                # Retrieve the pre-calculated math
                acc_diff = -metrics.get('acc_drop', 0.0) # Negative drop = positive diff
                acc_pct = metrics.get('acc_diff_pct', 0.0)
                lat_pct = metrics.get('lat_red', 0.0) * 100
                vram_pct = metrics.get('vram_red', 0.0) * 100
                emit_pct = metrics.get('emit_red', 0.0) * 100
...
"""
    ...
    def _create_prompt(self, iteration: int, trial_history: list, pareto: list = None,
                       weights: dict = None, rejected_configs: list = None, targets: dict = None) -> str:
        ...
        if targets:
            ...
            target_str += "The user has set the following hard targets. Prioritize achieving these minimum thresholds:\n"
            if targets.get("vram_pct", 0) > 0:
                target_str += f"- VRAM: Reduce by at least {targets['vram_pct']*100}% compared to baseline.\n"
            if targets.get("lat_pct", 0) > 0:
                target_str += f"- Latency: Reduce by at least {targets['lat_pct']*100}% compared to baseline.\n"
            if targets.get("emit_pct", 0) > 0:
                target_str += f"- Emissions: Reduce by at least {targets['emit_pct']*100}% compared to baseline.\n"
            if targets.get("acc_drop", 0) > 0:
                target_str += f"- Accuracy: Maximum allowed drop is {targets['acc_drop']} (absolute points).\n"
            target_str += "If these targets are aggressive, choose aggressive compression modes immediately.\n"
...
"""
    ...
    def get_suggestion(self, iteration: int, trial_history: list, pareto: list = None,
        ...
        prompt = self._create_prompt(iteration, trial_history, pareto=pareto, weights=weights, rejected_configs=rejected_configs, targets=targets)
        messages = [{"role": "user", "content": prompt}]
