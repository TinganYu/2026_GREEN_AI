...
from Global_Tuner_memory.modular_agent_new.schemas import StrategySuggestion
...
#- For quant_only: start with GPTQ 4-bit, then explore variations.
    ...
    def get_suggestion(self, iteration: int, trial_history: list, pareto: list = None,
                       weights: dict = None, rejected_configs: list = None, dedup_attempt: int = 0):
        """
        ...
        if self.memory_type == "tool":
            ...
            if len(trial_history) <= 3:
                ...
                try:
                    return self.get_suggestion(iteration, trial_history, pareto, weights, rejected_configs, dedup_attempt)
                finally:
                    self.memory_type = original
            return self._get_suggestion_with_tools(iteration, trial_history, pareto, weights, rejected_configs, dedup_attempt)
    ...
    def _get_suggestion_with_tools(self, iteration: int, trial_history: list, pareto: list = None, weights: dict = None, rejected_configs: list = None, dedup_attempt: int = 0):
                    ...
                    try:
                        ...
                        if attempt == 1: # Last attempt failed
                            ...
                            try:  # just use fallback memory mode to get a valid suggestion without crashing the whole system, even if it's not tool-optimized
                                return self.get_suggestion(iteration, trial_history, pareto, weights, dedup_attempt=dedup_attempt)
                            finally:
