...
import argparse
from Global_Tuner_memory.modular_agent_new.llm_client import LLMDecisionMaker
from Global_Tuner_memory.modular_agent_new.executors import run_asvd, run_sparse, run_quantization, run_evaluation
from Global_Tuner_memory.modular_agent_new.utils import get_pareto_frontier
...
class OptimizationOrchestrator:
    ...
    def optimize(self):
        ...
        for i in range(start_iter, self.max_iterations + 1):
            ...
            _MAX_DUP_RETRIES = 1#5
            suggestion, llm_output = None, None

            rejected_this_iter = []
            for _retry in range(_MAX_DUP_RETRIES):
                _s, _raw = self.llm.get_suggestion(i, self.trial_history, pareto=pareto, weights=self.weights, rejected_configs=rejected_this_iter, dedup_attempt=_retry)
