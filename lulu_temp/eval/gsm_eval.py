...
# #         # Use the final agent's config (AdaptiveKVController proxies it)
...
# #         """
# #         Robust extraction from LLM outputs.
# #         Strategy:
# #           1. Prefer last number after 'Answer:'.
# #           2. Else, extract all numbers and pick mode (if repeated) or last.
# #         """
# #         if not text:
# #             return None
...
# #         # Otherwise, fallback to all numbers
...
# #         #     "error": error
# #         # }
# #         # response is expected to be a Python str, but be defensive:
...
# #         # predicted, ground_truth are floats/None already; ensure Python numbers
...
# #         # minimal result dict with CPU/native types only
# #         result = {
# #             "question": question,
# #             "ground_truth": ground_truth,
# #             "predicted": predicted,
...
# #         # free local references used during generation (best effort)
# #         try:
# #             del response
# #         except Exception:
# #             pass
...
# #         try:
# #             # If agent has model loaded, delete it to free GPU memory for next tuning run
# #             if hasattr(self.agent, "model") and self.agent.model is not None:
# #                 try:
# #                     # move model to cpu then delete (safer)
# #                     try:
...
# #                 except Exception:
# #                     logger.warning("Failed to delete agent.model cleanly")
# #         except Exception:
# #             pass

# #         # collect Python gc and free CUDA caches / reset peaks
# #         gc.collect()
# #         if torch.cuda.is_available():
# #             try:
# #                 torch.cuda.empty_cache()
# #                 # free any held IPC caches (helpful with some drivers)
# #                 if hasattr(torch.cuda, "ipc_collect"):
# #                     torch.cuda.ipc_collect()
# #                 # reset peak memory counters so next run's peak is fresh
# #                 try:
# #                     torch.cuda.reset_peak_memory_stats(device=getattr(self.agent, "device", None) or torch.device("cuda:0"))
# #                 except Exception:
# #                     # fallback: call with no device
...
# #         else:
# #             # fallback: brute force grid search
# #             ratios = tuning_cfg.get('compression_ratios_to_test', [1.0])
# #             methods = tuning_cfg.get('compression_methods_to_test', ['magnitude'])
# #             all_results = []

# #             for method in methods:
# #                 for ratio in ratios:
# #                     logger.info(f"Tuning run: Method={method}, Ratio={ratio}")
# #                     # Sync agent settings to config
...
# #                     res['summary']['configuration'].update({'tuning_method': method, 'tuning_ratio': ratio})
...
# #             "nans": 0  # you could add detection here
# #         }
    
# #     def run_adaptive_tuning(self, samples=None, use_cot=True):
# #         """
# #         Adaptive tuning loop: process samples one by one, letting the agent adjust per-sample.
# #         """
# #         self.agent.load_model()
...
# #             # Update agent parameters based on per-sample feedback
...
# #             # Optional logging of current parameters
# #             if isinstance(self.agent, AdaptiveKVController):
...
# #         """Common cleanup to call between heavy runs (tuning/eval)"""
# #         import gc
# #         gc.collect()
# #         if torch.cuda.is_available():
# #             try:
...
# #                 if hasattr(torch.cuda, "ipc_collect"):
# #                     torch.cuda.ipc_collect()
# #                 # reset peaks for more consistent peak reporting in subsequent runs
# #                 try:
# #                     # prefer evaluator agent device if available
...
#         if use_compression and hasattr(self.agent, "get_compression_stats"):
#             kv_stats = self.agent.get_compression_stats()
#         else:
#             kv_stats = None
...
#         compressed_tokens = [r['kv_stats']['compressed_tokens'] 
#                         for r in results if r['kv_stats'] is not None]
#         original_tokens = [r['kv_stats']['original_tokens'] 
#                         for r in results if r['kv_stats'] is not None]

#         avg_compression_ratio = (sum(compressed_tokens)/sum(original_tokens)
#                                 if sum(original_tokens) > 0 else None)
...
#                         'tuning_ratio': ratio,
#                         'tokens_saved': kv_stats.get('tokens_saved'),
#                         'avg_compression_ratio': kv_stats.get('compression_ratio')
#                     })
#                     all_results.append(res['summary'])
