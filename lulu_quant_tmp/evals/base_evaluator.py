...
# 在載入 transformers/gptqmodel 之前就設好環境變數
os.environ.setdefault("GPTQMODEL_TRITON_DISABLE", "1")
os.environ.setdefault("GPTQ_USE_TORCH", "1")
...
class BaseEvaluator(ABC):
    ...
    #     def _force_torch_kernels(module):
    #         for name, child in list(module.named_children()):
    #             if isinstance(child, TritonV2QuantLinear):
    #                 # 取維度，fallback 由 qweight 推斷
    ...
    #                     qw = getattr(child, "qweight", None)
    #                     if qw is not None and qw.ndim >= 2:
    #                         out_f, in_f = qw.shape[0], qw.shape[1]
    #                     else:
    #                         raise RuntimeError(f"無法從 {child} 推斷維度")
    ...
    #                     out_features=out_f,
    #                     bias=False,
    #                     sym=getattr(child, "sym", False),
    #                     desc_act=getattr(child, "desc_act", None),
    #                 )
    #                 # ⚠️ 修正：必須先搬移張量，才能呼叫 dequantize_weight
    #                 t.qweight = child.qweight
    #                 t.qzeros  = child.qzeros
    #                 t.scales  = child.scales
    #                 t.g_idx   = child.g_idx
    #                 if child.bias is not None:
    #                     t.bias = child.bias

    #                 # 現在才呼叫，生成內部緩衝區 (wf_unsqueeze_zero 等)
    #                 try:
    #                     _ = t.dequantize_weight(num_itr=1)
    #                 except:
    #                     pass

    #                 setattr(module, name, t)
    #             else:
    #                 _force_torch_kernels(child)

    #     _force_torch_kernels(self.model)
    def _load_normal_model(self):
        """載入模型並自動處理註冊與維度問題"""
        logger.info("🔹 正在初始化 ASVD 模型架構...")
        
        from transformers import AutoConfig, AutoModelForCausalLM
        from transformers.models.auto.configuration_auto import CONFIG_MAPPING

        # 1. 讀取 Config (此時會根據 auto_map 載入 ASVDLlamaConfig)
        ...
        # 2. 【關鍵修正】強制變更 model_type 避開系統內建的 llama 邏輯
        # 這樣 AutoModel 就會完全依賴模型資料夾內的 auto_map 來找尋 ASVDLlamaForCausalLM
        config.model_type = "asvd_llama" 
        CONFIG_MAPPING._extra_content["asvd_llama"] = config.__class__
        
        logger.info(f"✅ 已將內部型號暫時更名為 asvd_llama 以啟用自定義架構")

        # 3. 載入模型 (一定要傳入這個修改過 model_type 的 config 物件)
        try:
            self.model = AutoModelForCausalLM.from_pretrained(
                self.config.model_path,
                config=config, 
            ...
            logger.error(f"❌ 模型載入基礎架構失敗: {e}")
            raise

        # 4. 處理 GPTQ 核心轉換（保留您原本成功的邏輯）
        # from gptqmodel.nn_modules.qlinear.tritonv2 import TritonV2QuantLinear
        from gptqmodel.nn_modules.qlinear.torch import TorchQuantLinear

        def _force_torch_kernels(module):
            for name, child in list(module.named_children()):
                # 關鍵：不要用 isinstance 檢查 Triton 類別，改用名稱字串檢查
                if "QuantLinear" in child.__class__.__name__:
                    in_f = getattr(child, "in_features", None) or getattr(child, "infeatures", None)
                    out_f = getattr(child, "out_features", None) or getattr(child, "outfeatures", None)
        ...
        # _force_torch_kernels(self.model)
        logger.info("✅ 已成功切換至 Torch 核心並完成模型初始化")
