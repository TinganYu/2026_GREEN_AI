...
class GPTQConfig(BaseQuantConfig):
    ...
    calib_num: int = 256
    dynamic: Optional[dict] = field(default=None) # 新增此行以支援動態配置
    use_eora: bool = field(default=False)
    eora_rank: int = field(default=32)
    eora_dataset: str = field(default="openai/gsm8k")
...
class Quantizer:
    ...
    def _quantize_gptq(self, config: GPTQConfig, trial_number: int = None, exp_dir: str = None) -> str:
        ...
        logger.info("   ✓ 模型載入完成")
        # # Flatten: replace ASVDLinear → BLinear, save ALinear aside
        # import glob
        # saved_alinears = self._flatten_asvd_for_gptq(model)
        # logger.info(f"   Flattened {len(saved_alinears)} ASVD layers for quantization")
        ...
        self._patch_gptqmodel_for_asvd(model) # 針對 ASVD 模型的特殊處理
        ...
        logger.info("   ✓ 儲存完成")
        # # Restore: rename keys to BLinear.*, add ALinear weights, patch config
        # self._restore_and_fix_checkpoint(
        #     output_dir, 
        #     saved_alinears, 
        ...
        if config.use_eora:
            self._generate_eora(
                ...
                config=config,
                calibration_dataset=calibration_dataset # 重用量化時的 dataset
            )
    ...
    def _quantize_awq(self, config: AWQConfig, trial_number: int = None, exp_dir: str = None) -> str:
        ...
        model = AutoModelForCausalLM.from_pretrained(  #AutoAWQForCausalLM
            self.model_path,
        ...
        logger.info("🔹 開始量化（使用 oneshot + AWQModifier，支援 ASVD 層）...")

        # 建立 AWQ mapping，支援 Alinear/Blinear 拆分後的 self-attn 層
#         awq_mapping = [
#             # Atteawq_mapping = [
#     # Input layernorm -> Q/K/V
#     AWQMapping(
#         "re:.*input_layernorm",
#         ["re:.*q_proj.*", "re:.*k_proj.*", "re:.*v_proj.*"],
#     ),
#     # v_proj variants -> O projection
#     AWQMapping("re:.*v_proj$", ["re:.*o_proj.*"]),        # matches the base v_proj
#     AWQMapping("re:.*v_proj\\.ALinear$", ["re:.*o_proj.*"]),  # matches ALinear
#     AWQMapping("re:.*v_proj\\.BLinear$", ["re:.*o_proj.*"]),  # matches BLinear
...
# ]
        from llmcompressor.modifiers.awq.mappings import _default_mappings
        extended_mappings = []

        for mapping in _default_mappings:
            smooth = mapping.smooth_layer
            balance = mapping.balance_layers

            # Modify only projection-related layers
            if "v_proj" in smooth:
            ...
            #             "v_proj(\.BLinear)?$"
            #         )
            #     new_balance.append(b)

            extended_mappings.append(
                AWQMapping(
                    smooth_layer=smooth,
                    balance_layers=new_balance
                )
            )
        ...
        # # 替換原本 mapping 的 regex，支援 ASVD 層
        # for m in _default_mappings:
        #     m.smooth_layer = m.smooth_layer.replace("$", "")   # 去掉 $ 結尾
        #     m.balance_layers = [b.replace("$", ".*") for b in m.balance_layers]

        # 準備 calibration dataset (可使用小 sample)
        from datasets import Dataset
        MAX_SEQ_LENGTH = 512
        logger.info("🔹 準備校準資料（使用 GSM8K 範例）...")
        NUM_CALIBRATION_SAMPLES = 256  # 或固定 256
        raw_ds = load_dataset("openai/gsm8k", "main", split="train").select(range(NUM_CALIBRATION_SAMPLES))

        # 轉成 oneshot 可用的格式
        calibration_dataset = Dataset.from_dict({
            "text": [ex["question"] + " " + ex["answer"] for ex in raw_ds]
        })
        logger.info(f"   ✓ 準備了 {len(calibration_dataset)} 個校準樣本")
        # # before post_process
        # if not hasattr(model, "save_pretrained"):
        #     model.save_pretrained = model.save_quantized
...
##################################################################################################################################
    ...
    def _flatten_asvd_for_gptq(self, model):
        """
        調包計：將 BLinear 扶正，並用 Hook 隱藏 ALinear 的運算。
        """
        saved_alinears = {}
        
        # 遍歷所有層，尋找 ASVDLinear 容器
        for name, module in list(model.named_modules()):
            if module.__class__.__name__ == "ASVDLinear":
                # 找到父層（例如 self_attn）
                parent_path = ".".join(name.split(".")[:-1])
                child_name = name.split(".")[-1]
                parent = model.get_submodule(parent_path)
                
                b_layer = module.BLinear # 零件 B (要被量化的部分)
                a_layer = module.ALinear # 零件 A (負責還原維度的部分)
                
                # 🔹 核心黑科技：Forward Hook
                # 這是為了保證模型在量化推理時，輸出永遠是 3072
                # 這樣 Attention (24 heads) 就不會報 shape 錯誤
                def restore_dim_hook(mod, inp, out, a=a_layer):
                    return a(out)
                
                b_layer.register_forward_hook(restore_dim_hook)
                
                # 🔹 將 B 零件取代盒子的位置
                # 這樣 GPTQ 看到的就是 model.layers.0.self_attn.k_proj (它是 nn.Linear)
                setattr(parent, child_name, b_layer)
                
                # 紀錄完整路徑，方便量化後還原權重
                saved_alinears[name] = a_layer
                
        logger.info(f"✅ 成功偽裝了 {len(saved_alinears)} 個層。現在 GPTQ 認得它們，且 Attention 維度正常")
        return saved_alinears

    def _restore_and_fix_checkpoint(self, output_dir, saved_alinears, original_config_path):
        """After save_quantized(), fix the safetensors to have BLinear keys and add ALinear."""
        from safetensors.torch import load_file, save_file
        import json, shutil
        import glob

        shard_files = sorted(glob.glob(os.path.join(output_dir, "*.safetensors")))
        ...
        for shard_path in shard_files:
            ...
            for key, tensor in state.items():
                # 假設 key 是 model.layers.0.self_attn.q_proj.qweight
                # 我們要把它變成 model.layers.0.self_attn.q_proj.BLinear.qweight
                is_asvd_key = any(key.startswith(path) for path in saved_alinears.keys())
                if is_asvd_key:
                    new_key = key.replace(".qweight", ".BLinear.qweight")\
                                .replace(".qzeros", ".BLinear.qzeros")\
                                .replace(".scales", ".BLinear.scales")\
                                .replace(".g_idx", ".BLinear.g_idx")
                    new_state[new_key] = tensor
            
            # Add ALinear weights to this shard (put them all in the first shard)
            if shard_path == shard_files[0]:
                for asvd_name, alinear in saved_alinears.items():
                    new_state[f"{asvd_name}.ALinear.weight"] = alinear.weight.data.cpu()
                    if alinear.bias is not None:
                        new_state[f"{asvd_name}.ALinear.bias"] = alinear.bias.data.cpu()
            
            save_file(new_state, shard_path)
        ...
        saved["architectures"] = ["ASVDLlamaForCausalLM"]
        saved["auto_map"] = original["auto_map"]
        saved["truncation_ranks"] = original["truncation_ranks"]
        
        with open(os.path.join(output_dir, "config.json"), "w") as f:
            json.dump(saved, f, indent=2)
        
        # Copy custom model files
        model_dir = os.path.dirname(original_config_path)
        for fname in ["configuration_asvd_llama.py", "modeling_asvd_llama.py"]:
            src = os.path.join(model_dir, fname)
            if os.path.exists(src):
                shutil.copy(src, output_dir)

    def _generate_eora(self, model_id: str, quant_path: str, config: GPTQConfig, calibration_dataset: list):
        """生成 EoRA Adapter 以恢復精度"""
        try:
            from gptqmodel import GPTQModel, Lora
        except ImportError:
            logger.error("❌ 無法載入 GPTQModel 進行 EoRA 生成")
            return

        logger.info(f"✨ 開始生成 EoRA (Rank: {config.eora_rank})...")
        
        eora_save_path = os.path.join(quant_path, f"eora_rank{config.eora_rank}")
        
        eora_adapter = Lora(
            path=eora_save_path,
            rank=config.eora_rank
        )

        # 呼叫 GPTQModel 的 adapter 生成接口
        ...
        logger.info(f"✅ EoRA 生成完成，儲存於: {eora_save_path}")

    def _patch_gptqmodel_for_asvd(self,model):
        """
        告訴 GPTQ 直接去抓 BLinear 層進行量化，避開 ASVDLinear 包裝
        """
        ModelClass = type(model)
        # LlamaGPTQ.inside_layer_modules 是 GPTQ 內部用來找層的清單
        # 我們將其路徑加上 .BLinear
        ModelClass.inside_layer_modules = [
    ["self_attn.q_proj", "self_attn.k_proj", "self_attn.v_proj",
     "self_attn.q_proj.BLinear", "self_attn.k_proj.BLinear", "self_attn.v_proj.BLinear"],
    ["self_attn.o_proj", "self_attn.o_proj.BLinear"],
    ["mlp.gate_proj", "mlp.up_proj", "mlp.gate_proj.BLinear", "mlp.up_proj.BLinear"],
    ["mlp.down_proj", "mlp.down_proj.BLinear"],
        ]
        logger.info("✅ 已修正 GPTQ 掃描路徑以支援 ASVD 結構")