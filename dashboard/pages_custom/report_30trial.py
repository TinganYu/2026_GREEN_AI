...
st.set_page_config(
    page_title="Green AI Tuner Dashboard (30 Trial)",
    page_icon="🌿",
    layout="wide",
)
...
分析涵蓋 7 種搜尋策略：**LLM-Full**、**LLM-Summary**、**LLM-Window**、**Tool Agent**、**TPE**、**NSGA-II**、**Random Search**，每種各執行 3 次，共 21 組實驗。
""")
...
在 30 個 trial 的探索中，各方法都踩過以下地雷——但記憶機制強的方法踩得更少：

| 危險配置 | 典型 Accuracy | 觀察到的 Catastrophic 率 |
|---------|-------------|----------------------|
| `qqq 4-bit g128` | 1%～2% | 所有方法均有，屬搜尋空間必踩點 |
| `sparse 結構化 2:4` | 6%～8% | 幾乎每次執行都出現一次 |
| `asvd_only ratio≤0.9` | 15%～27% | 某些參數下危險，視 alpha 而定 |
| `hybrid_asvd_bnb` | 13%～22% | 組合策略不穩定，分數方差大 |

**LLM-Window 的災難性失敗率最低（3.3%）**，原因是詳細歷史讓 LLM 能精確比對哪些 alpha/ratio 值危險，避免重蹈覆轍。
Tool Agent（6.7%）最差，因其不維護完整歷史，依賴每次的 context window，容易遺忘已知失敗模式。
""")
    ...
    if not df_report.empty:
        pos_rows = df_report.set_index("方法")["正向率_num"].to_dict()
        best_pos_method = max(pos_rows, key=pos_rows.get)
        worst_pos_method = min(pos_rows, key=pos_rows.get)
        best_pos_val = pos_rows[best_pos_method]
        worst_pos_val = pos_rows[worst_pos_method]
        diff_pp = best_pos_val - worst_pos_val
        best_pos_count = round(30 * best_pos_val / 100)
        worst_pos_count = round(30 * worst_pos_val / 100)
        st.markdown(f"""
**{best_pos_method}** 的正向 Trial 率最高（**{best_pos_val:.1f}%**），
**{worst_pos_method}** 最低（**{worst_pos_val:.1f}%**），差距 **{diff_pp:.1f} 個百分點**。

換句話說：在同樣 30 次試驗的預算下——
- {best_pos_method} 平均有 **{best_pos_count} 個**有效 trial（score > 0）
- {worst_pos_method} 平均只有 **{worst_pos_count} 個**

每一次有效 trial 都在推進對最佳壓縮配置的理解。
無效 trial 不只浪費 GPU 時間，更直接等於不必要的碳排放——Green AI 場景下雙重損失。
""")
    ...
    # 動態生成收斂分析
    if conv_data:
        conv_rows_sorted = sorted(conv_data.items(), key=lambda x: x[1]["avg_first_best"])
        fastest_name, fastest_d = conv_rows_sorted[0]
        slowest_name, slowest_d = conv_rows_sorted[-1]
        st.markdown(f"""
**{fastest_name}** 平均在第 **{fastest_d['avg_first_best']:.1f}** 個 trial 就找到最終最佳配置，
**{slowest_name}** 則需要等到第 **{slowest_d['avg_first_best']:.1f}** 個，差距約 **{slowest_d['avg_first_best']-fastest_d['avg_first_best']:.1f} 個 trial**。

這差距的根本原因：
- **記憶方法**（Full/Summary/Window）在第一次探索所有 8 種壓縮模式後，迅速確認 GPTQ 為主方向，後續 trial 專注超參數精調。
- **Tool Agent** 不維護跨 session 記憶，每次決策依賴當前 context，探索效率較低，平均需要更多 trial 才能鎖定最佳方向。

> LLM 的先驗知識讓它在 **Iteration 1** 就優先嘗試 GPTQ/AWQ 等安全量化方法，
> 而不是從隨機起點慢慢排除高風險選項。
""")
    ...
    st.markdown("### 5.1 系統性覆蓋式探索策略（Iter 1～8）")
    st.markdown("""
所有 LLM 記憶方法在前 8 個 trial 中，幾乎都覆蓋了所有 8 種壓縮類型（各試一次），且每次都有明確的選擇理由。
這不是隨機的廣度探索，而是**有組織的搜尋空間覆蓋**——與 Tool Agent 的差異在於記憶讓 LLM 知道「還有哪些沒試」。
""")

    @st.cache_data
    def compute_exploration_table() -> pd.DataFrame:
        """從 LLM-Summary Run1 的實際資料動態建立探索表格。"""
        def _glob_runs(pattern):
            return sorted(p.name for p in BASE_DIR.glob(pattern) if (p / "optimization_results.json").exists())
        summary_folders = _glob_runs("exp_Llama-3.2-3B-Instruct_gsm8k_summary_*")
        if not summary_folders:
            return pd.DataFrame()
        data = json.load(open(BASE_DIR / summary_folders[0] / "optimization_results.json", "r", encoding="utf-8"))
        rows = []
        for d in data[:10]:
            ...
            rows.append({
                ...
                "壓縮方法":   d.get("config", {}).get("mode", "?"),
                "LLM 推理摘要": reasoning[:120] + "…" if len(reasoning) > 120 else reasoning,
                "Score":     round(m.get("score", 0), 3),
                "Accuracy":  round(m.get("accuracy", 0), 4),
            })
        return pd.DataFrame(rows)

    df_expl = compute_exploration_table()
    ...
    st.caption("資料來源：LLM-Summary Run1 實際執行結果（動態載入）")

    st.markdown("### 5.2 學習失敗、建立機制性解釋（全部 12 個 LLM 實驗）")
    st.markdown("""
此節動態載入所有 12 個 LLM 實驗（Full × 3、Summary × 3、Window × 3、Tool Agent × 3）的原文推理，
依「決策模式」分 4 個主題整理，讓你看到**跨方法、跨次執行的一致性**，而不只是單次結果。
""")

    @st.cache_data
    def load_all_llm_reasoning() -> dict:
        """Load reasoning from all LLM experiments, keyed by (exp_type, run_id, iter)."""
        def _glob_runs(pattern: str) -> dict:
            folders = sorted(p.name for p in BASE_DIR.glob(pattern) if (p / "optimization_results.json").exists())
            return {f"Run{i+1}": name for i, name in enumerate(folders)}
        llm_exps = {
            **{("Full",       rid): folder for rid, folder in _glob_runs("exp_Llama-3.2-3B-Instruct_gsm8k_full_*").items()},
            **{("Summary",    rid): folder for rid, folder in _glob_runs("exp_Llama-3.2-3B-Instruct_gsm8k_summary_*").items()},
            **{("Window",     rid): folder for rid, folder in _glob_runs("exp_Llama-3.2-3B-Instruct_gsm8k_window_*").items()},
            **{("Tool Agent", rid): folder for rid, folder in _glob_runs("exp_Llama-3.2-3B-Instruct_gsm8k_tool_*").items()},
        }
        result = {}
    ...
    # ── 主題一：失敗後立即反應 ─────────────────────────────────────────────────
    st.markdown("#### 🔴 主題一：遭遇失敗後的即時反應與策略調整（Iter 3～6）")
    st.markdown("""
ASVD 或 sparse 在 Iter 2～5 導致 accuracy 崩潰後，LLM 在**下一個 iteration 立刻做出清晰的診斷**，
並切回量化方法（GPTQ/AWQ/BNB）。記憶型方法（Full/Summary/Window）的反應比 Tool Agent 更一致，
因為記憶讓 LLM 明確「看到」失敗記錄，而非只依賴 context window 的隱含推斷。
""")
    _t1a, _t1b, _t1c, _t1d = st.tabs(["🔵 Full", "📄 Summary", "🪟 Window", "🔧 Tool Agent"])
    for _tab, _et, _iters in [
        (_t1a, "Full",       [3, 4]),
        (_t1b, "Summary",    [3, 4]),
        (_t1c, "Window",     [3, 4]),
        (_t1d, "Tool Agent", [3, 4]),
        ...
        with _tab:
            ...
            for _col, _rid in zip([_c1, _c2, _c3], ["Run1", "Run2", "Run3"]):
                with _col:
                    st.markdown(f"**{_et}-{_rid}**")
                    show_reasoning_block(_et, _rid, _iters)
    st.info("""
**跨 12 次執行的共同模式**：所有方法在 ASVD/sparse 造成 accuracy 崩潰後，下一個 iter 幾乎都切回 GPTQ/AWQ/BNB。
記憶方法（Full/Summary/Window）的推理更清晰——能明確引用「上次 ASVD 的 accuracy = 0.18」；
Tool Agent 的診斷更模糊，有時誤判失敗原因，甚至再踩一次類似的雷。
""")

    # ── 主題二：全模式探索完後的收斂認識 ─────────────────────────────────────
    st.markdown("#### 🟡 主題二：全模式探索完畢後的 GPTQ 收斂（Iter 9～12）")
    st.markdown("""
在前 8 個 trial 涵蓋所有壓縮類型後，記憶型 LLM 幾乎**全部在 Iter 9～12 得出相同結論**：
「GPTQ 4-bit 分數最高，下一步聚焦精調超參數。」
**Tool Agent** 則因為沒有完整歷史，有時在 Iter 9 仍在嘗試 Hybrid/ASVD——收斂更慢。
""")
    _t2a, _t2b, _t2c, _t2d = st.tabs(["🔵 Full", "📄 Summary", "🪟 Window", "🔧 Tool Agent"])
    for _tab, _et, _iters in [
        (_t2a, "Full",       [9, 10]),
        (_t2b, "Summary",    [9, 10]),
        (_t2c, "Window",     [9, 10]),
        (_t2d, "Tool Agent", [9, 10]),
    ]:
        with _tab:
            _c1, _c2, _c3 = st.columns(3)
            for _col, _rid in zip([_c1, _c2, _c3], ["Run1", "Run2", "Run3"]):
    ...
    st.markdown("""
記憶型 LLM 不只記住「哪個方法分數低」，還會**推斷失敗機制**：
例如「ASVD+BNB 存在相容性問題」、「激進 SVD 壓縮必然造成大量精度損失」、
「QQQ 在此模型架構上尚未驗證」。Iter 10-14 的推理最能體現這種機制性理解。
""")
    _t3a, _t3b, _t3c, _t3d = st.tabs(["🔵 Full", "📄 Summary", "🪟 Window", "🔧 Tool Agent"])
    for _tab, _et, _iters in [
        (_t3a, "Full",       [10, 11, 12]),
        (_t3b, "Summary",    [10, 11, 12]),
        (_t3c, "Window",     [10, 11, 12]),
        (_t3d, "Tool Agent", [10, 11, 12]),
    ]:
        with _tab:
            _c1, _c2, _c3 = st.columns(3)
            for _col, _rid in zip([_c1, _c2, _c3], ["Run1", "Run2", "Run3"]):
    ...
    # ── 主題四：超參數精調（Iter 15+）────────────────────────────────────────
    st.markdown("#### 🟢 主題四：超參數安全區間的自動歸納與精調（Iter 15+）")
    st.markdown("""
在後期（Iter 15-30），記憶型 LLM 從多次 GPTQ 實驗中**自動歸納出安全超參數區間**
（如 `damp_percent ≈ 0.006～0.02` 表現最穩定），並在此區間做細密搜尋。
**Full 模式**（看到全部 30 個 trial 的完整記錄）理論上最有利於後期精調——
但實際資料顯示，30 個 trial 的後 10 個（21-30）對最終分數改善**有限**（平均 delta ≈ +0.006）。
""")
    _t4a, _t4b, _t4c, _t4d = st.tabs(["🔵 Full", "📄 Summary", "🪟 Window", "🔧 Tool Agent"])
    for _tab, _et, _iters in [
        (_t4a, "Full",       [15, 16, 17, 18]),
        (_t4b, "Summary",    [15, 16, 17, 18]),
        (_t4c, "Window",     [15, 16, 17, 18]),
        (_t4d, "Tool Agent", [15, 16, 17, 18]),
    ]:
        with _tab:
            _c1, _c2, _c3 = st.columns(3)
            for _col, _rid in zip([_c1, _c2, _c3], ["Run1", "Run2", "Run3"]):
    ...
    st.markdown("以下統計各次執行中，哪個 iteration 首次做出關鍵決策轉折點：")
    ...
    def compute_decision_patterns() -> pd.DataFrame:
        ...
        for (exp_type, run_id), folder in llm_exps.items():
            ...
            early_best = max(scores[:20]) if len(scores) >= 20 else best_score
            late_best  = max(scores[20:]) if len(scores) > 20 else best_score
            rows.append({
                "實驗":             f"{exp_type}-{run_id}",
                "類型":             exp_type,
                "首次 GPTQ (iter)": first_gptq or "—",
                "失敗後回量化":     first_recovery or "—",
                "連續 GPTQ 起始":   first_gptq_streak or "—",
                "達到最佳 (iter)":  best_iter or "—",
                "最終最佳 Score":   round(best_score, 3),
                "正向 Trial 數":    sum(1 for s in scores if s > 0),
                "後10試改善":       round(late_best - early_best, 3),
            })
        return pd.DataFrame(rows)
    ...
    if df_patterns.empty:
        st.caption("（找不到對應實驗資料夾，無法顯示決策模式表）")
    else:
        style_pat = (
            df_patterns.style
            .background_gradient(subset=["最終最佳 Score"], cmap="RdYlGn")
            .background_gradient(subset=["正向 Trial 數"], cmap="Blues")
            .background_gradient(subset=["後10試改善"], cmap="RdYlGn",
                                 vmin=-0.5, vmax=0.2)
            .set_table_styles([
                {"selector": "th", "props": [("background-color", "#1a252f"), ("color", "white"),
    ...
    st.markdown("""
**觀察重點（基於實際資料）**：
- **首次 GPTQ**：Full/Window 多數在 Iter 1-2，Summary 視 Run 而定；Tool Agent 偶爾延遲到 Iter 3
- **失敗後回量化**：記憶型方法（Full/Summary/Window）通常在下一個 iter 就修正；Tool Agent 有時連踩兩雷
- **連續 GPTQ 起始**：記憶型方法約 Iter 10-15；Tool Agent 較晚（約 Iter 15-20）
- **後10試（21-30）改善**：多數微幅或略降——30 trial 後段的邊際收益低，20 trial 已捕捉大部分價值
""")

    st.markdown("### 5.3 壓縮方法品質排序（基於 120 次實際測量）")
    st.markdown("根據 12 組 × 30 trial = 360 次實驗的實際結果，各壓縮方法的表現如下：")

    method_quality_data = {
        "壓縮方法": [
            "GPTQ 4-bit g128", "AWQ 4-bit", "BNB 4-bit",
            "ASVD ratio≥0.95", "ASVD ratio≈0.9",
            "Hybrid ASVD+BNB", "Sparse unstructured 50%", "Sparse 2:4", "QQQ 4-bit",
        ],
        "典型 Accuracy": [
            "66%～68%", "62%～67%", "66%～69%",
            "25%～50%", "18%～28%",
        ...
        "LLM 策略": [
            "首要目標，後期精調超參數", "早期廣探，分數穩定", "廣探階段必試，VRAM 友好",
            "保守嘗試（ratio≥0.95），高 ratio 風險較低", "多次碰壁後規避",
    ...
    st.markdown("### 6.1 四種 LLM 記憶策略的本質差異")

    @st.cache_data
    def compute_strategy_std() -> dict:
        groups = {
        ...
        for name, pat in groups.items():
            ...
            for p in BASE_DIR.glob(pat):
                fp = p / "optimization_results.json"
                if fp.exists():
            ...
            if len(bests) >= 2:
                result[name] = round(statistics.stdev(bests), 4)
            elif bests:
                result[name] = 0.0
        return result

    stdev_map = compute_strategy_std()

    col_a, col_b, col_c, col_d = st.columns(4)
    for _col, _name in zip([col_a, col_b, col_c, col_d],
                            ["LLM-Full", "LLM-Summary", "LLM-Window", "Tool Agent"]):
        _std = stdev_map.get(_name, 0)
        _col.metric(f"{_name} Best Score Std", f"±{_std:.4f}")
...
| 策略 | 記憶機制 | 優勢 | 限制 |
|------|---------|------|------|
| **Full** | 完整保留所有 trial | 最高正向率（66.7%），後期精調能力最強 | Context 隨迭代增長，Token 成本高 |
| **Summary** | 歸納摘要後壓縮 | 最快收斂（Iter 10.7），計算效率高 | 摘要可能遺失細節，偶有決策失誤 |
| **Window** | 滑動視窗保留近期 | 最低災難失敗率（3.3%） | 近期偏誤，遠期記憶消退 |
| **Tool Agent** | 工具調用取代記憶 | 不依賴 context 長度 | 決策一致性最差，正向率最低（46.7%） |
""")

    st.markdown("### 6.2 最佳配置的效益分析（Green AI 角度）")
    st.markdown("""
所有 LLM 方法最終都收斂到相同結論：**GPTQ 4-bit g128，damp_percent ≈ 0.006～0.02**
是 Llama-3.2-3B-Instruct 在 GSM8K 任務上的最優壓縮配置。
""")
    ...
    # 使用實際觀測到的最佳值（LLM-Full Run3 最高 score 那筆）
    best_lat  = 472.3
    best_vram = 2.32
    best_emit = 0.01444
    best_acc  = 0.684   # LLM-Full 最佳 trial 的 accuracy
    ...
    c4.metric("碳排放降低", f"{(b_emit - best_emit) / b_emit * 100:.1f}%",
              delta=f"−{b_emit - best_emit:.4f}", help="效益")
...
""")
    ...
    st.markdown("### 6.3 四種 LLM 記憶策略本質差異對比")
    comparison_data = {
        "比較維度": [
            "記憶機制", "先驗知識品質", "失敗處理速度",
            "收斂速度", "正向 Trial 率", "災難失敗率",
            "30 trial 後段收益", "可解釋性",
        ],
        "LLM-Full（完整歷史）": [
            "✅ 完整保留所有 trial",
...
- ⚠️ **Tool 模式的 EXACT/隱藏拆分特別不可信**：Tool 模式每次重新呼叫都會觸發自己的
  多輪工具檢索，但 `tool_debug_log.jsonl` 只記錄 iteration 編號、不記錄「這是第幾次
  重試」，導致部分本該算進「隱藏重試」的工具檢索成本，被錯誤地算進了「EXACT」裡
  （實測：某個 30-iteration 的 run 裡，理論上限應該 ≤30 次嘗試，實際卻偵測到 52 次，
  代表有超過 20 次重試輪次的工具呼叫被誤標成可信數字）。full/summary/window 三個模式
  沒有這個問題，因為它們的 EXACT 是直接從 `optimization_results.json` 存的最終建議反推，
  一個 iteration 對應一次呼叫，沒有這種模糊地帶。
""")
    ...
    # 動態計算最佳策略推薦
    if not df_report.empty:
        best_pos_m   = df_report.loc[df_report["正向率_num"].idxmax(), "方法"]
        lowest_cat_m = df_report.loc[df_report["災難率_num"].idxmin(), "方法"]
        fastest_conv_m = min(conv_data, key=lambda k: conv_data[k]["avg_first_best"]) if conv_data else "LLM-Summary"
    else:
        best_pos_m, lowest_cat_m, fastest_conv_m = "LLM-Full", "LLM-Window", "LLM-Summary"

    st.success(f"""
**四種 LLM 記憶策略的核心競爭力（基於 30-trial 實際數據）**

1. **最高正向率 → {best_pos_m}（66.7%）**：每 3 個 trial 就有 2 個有效（score > 0），
   GPU 時間與碳排放的浪費最少——Green AI 場景的最佳選擇之一。

2. **最快收斂 → {fastest_conv_m}（平均 Iter {conv_data[fastest_conv_m]['avg_first_best']:.1f}）**：
   10 個 trial 內就找到最終最佳配置，適合計算預算極為緊縮的場景。

3. **最安全 → {lowest_cat_m}（災難失敗率僅 3.3%）**：
   對 GPU 時間昂貴或不允許模型完全崩潰的生產部署場景最為穩健。

4. **所有方法的共識**：GPTQ 4-bit g128 是最優配置，
   能以 **約 5.8% 準確率損失** 換取 **62.5% VRAM + 66.8% Latency + 84.6% Emissions** 的節省。
""")

    st.warning(f"""
**限制與注意事項（基於實際觀測）**

- **30 trial 的後段（21-30）效益有限**：大多數方法的平均分數改善接近零或略降，
  代表在 20 trial 以內已捕捉到大部分搜尋價值。增加 trial 預算的邊際效益遞減明顯。

- **Tool Agent 在所有維度均墊底**：若需要 LLM 輔助搜尋，優先考慮記憶型方法（Full/Summary/Window）。

- **LLM-Summary 的 Run2 有異常**：Best Score 在 Iter 14 後反而下降（delta = −0.391），
